...
LINEAGE_ROOT = os.environ.get("LINEAGE_ROOT", "/home/ikan/lineage")


def run_build():
    cmd = f"bash -c 'cd {LINEAGE_ROOT} && source build/envsetup.sh && lunch lineage_P661N-bp4a-userdebug >/dev/null 2>&1 && m nothing 2>&1'"
...
def check_in_all_files(module_name, all_paths):
    ...
    candidates = [
        f"vendor/lib64/{file_name}",
        f"vendor/lib/{file_name}",
        f"vendor/lib64/lib{file_name}",
        f"vendor/lib/lib{file_name}",
    ]
...
def update_proprietary_files(new_blobs):
    ...
    header = (
        ["# Auto-added by fix-missing-blobx\n"]
        + [f"{b}\n" for b in sorted(new_blobs)]
        + ["\n"]
    )
...
def run_extraction():
    ...
    extract_py = os.path.join(parent_dir, "extract-files.py")
    setup_py = os.path.join(parent_dir, "setup-makefiles.py")

    if os.path.exists(extract_py):
        print("Running extract-files.py...")
        subprocess.run([extract_py, DUMP_DIR], check=False, cwd=parent_dir)

    if os.path.exists(setup_py):
        print("Running setup-makefiles.py...")
        subprocess.run([setup_py], check=False, cwd=parent_dir)
